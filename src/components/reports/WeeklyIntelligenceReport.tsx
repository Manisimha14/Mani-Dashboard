...
function MetricCard({ metric }: { metric: ReportMetricCard }) {
  ...
  return (
    <div style={{ ...glassCard, padding: '18px 18px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
...
function RingProgress({ current, total, label, valueLabel, accent }: { current: number; total: number; label: string; valueLabel: string; accent: string }) {
  ...
  return (
    <div style={{ ...glassCard, padding: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20 }}>
      <div>
...
function CodingHeatmap({ points }: { points: DailyReportPoint[] }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${points.length}, minmax(0, 1fr))`, gap: 12 }}>
      {points.map((point) => {
        ...
        return (
          <div key={point.date} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 54, height: 54, borderRadius: 18, background, border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900 }}>
...
function WorkoutTimeline({ points }: { points: DailyReportPoint[] }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {points.map((point) => (
        <div key={point.date} style={{ ...glassCard, padding: '14px 16px', display: 'grid', gridTemplateColumns: '92px 1fr auto', gap: 14, alignItems: 'center' }}>
...
function ComparisonTable({ rows }: { rows: WeeklyComparisonRow[] }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 14 }}>
      {rows.map((row) => (
        <div key={row.id} style={{ ...glassCard, padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
...
function InsightBlock({ report }: { report: WeeklyReportStats }) {
  if (report.validatedInsights.length === 0) {
    return (
      <div style={{ ...glassCard, padding: 22, display: 'flex', alignItems: 'center', gap: 12 }}>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${report.validatedInsights.length}, minmax(0, 1fr))`, gap: 14 }}>
      {report.validatedInsights.map((insight) => (
        <div key={insight.id} style={{ ...glassCard, padding: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
...
function TrackerCards({ cards }: { cards: CustomTrackerWeeklySummary[] }) {
  if (cards.length === 0) {
    return (
      <div style={{ ...glassCard, padding: 18, color: mutedText, fontSize: 12 }}>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 14 }}>
      {cards.map((card) => {
        ...
        return (
          <div key={card.trackerId} style={{ ...glassCard, padding: 18, display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {report.codingAnalytics.spacedRepetitionQueue.length === 0 ? (
              ...
              <div key={`${item.name}-${item.date}`} style={{ padding: '10px 12px', borderRadius: 16, background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(167,139,250,0.14)' }}>
                <div style={{ fontSize: 11, color: '#f8fafc', fontWeight: 700 }}>{item.name}</div>
                <div style={{ fontSize: 10, color: mutedText, marginTop: 4 }}>{item.difficulty} • {item.topic} • {item.date}</div>
              </div>
            ))}
