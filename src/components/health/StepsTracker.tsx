...
export default function StepsTracker({ today }: { today: string }) {
  ...
  // Google Fit States
  const [isFitConnected, setIsFitConnected] = useState(() => localStorage.getItem('google_fit_connected') === 'true');
          ...
          {isFitConnected ? (
            <>
              <motion.button
                ...
                className="px-4 py-2 rounded-xl text-xs font-bold bg-blue-500/10 text-blue-400 border border-blue-500/25 flex items-center gap-1.5 hover:bg-blue-500/20 transition-all shadow-[0_0_12px_rgba(59,130,246,0.1)]"
              >
                🔄 Sync Now
              </motion.button>
              <button
        ...
        {showOAuthModal && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="w-full max-w-md bg-[#1c1d21] border border-white/10 rounded-3xl p-6 shadow-[0_24px_50px_rgba(0,0,0,0.5)] relative overflow-hidden"
            >
              {/* Top Google Header */}
              ...
              <div className="flex gap-2.5 justify-end">
                <button 
                  ...
                  className="px-4 py-2 rounded-xl text-xs font-semibold text-white/40 hover:text-white/70 hover:bg-white/5 transition-all"
                >
                  Cancel
                </button>
                <motion.button 
            ...
            >
              ...
              <div className="space-y-2.5 text-left mb-2">
                {[
                  ...
                  'Pulling steps telemetry (9,420 steps)...',
                  'Extracting active movement (45m walking)...',
                  'Synchronizing and updating Supabase DB...'
                ].map((text, idx) => {
